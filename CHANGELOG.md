...
`memarium doctor` now checks the cross-device overlay (`~/.memarium/aggregated`,
...
recent sibling-device memory"), with `memarium sync` as the fix. Offline-safe —
...
- `src/project-identity.ts`: `canonicalProjectId()` collapses ssh-scp /
  ...
  (`github.com-june9593-memarium`), remote-first with path fallback.
...
> v0.8.x.** Run `memarium config --encrypt false` then `memarium sync` to
> re-commit `raw_sessions/` as plaintext and push. Once you're on 0.10.0,
> memarium no longer wires the git-crypt filter, so any commits still in
> ciphertext become unreadable on checkout.

After upgrading, `~/.memarium/passphrase` and `.memarium/repo-salt.json`
are inert — memarium never reads them again. You can delete them manually;
...
### Cross-device typed memory (memarium Memory OS v1)

The memarium **plugin** 0.3.0 introduces a typed-memory layer: durable
`memory/<type>/<scope>/<slug>.md` files + a `.memarium/index.memory.json`
...
typed-memory layer into the working tree but doesn't push. `memarium sync`
now also stages `memory/` + `.memarium/index.memory.json` onto the device
...
### `memarium resume` — size-adaptive prompt construction
...
### `memarium doctor` — multi-install detection

A user can end up with `memarium` installed simultaneously to multiple
npm prefixes (Homebrew's `/opt/homebrew/lib/node_modules/` AND nvm's
`~/.nvm/versions/node/<v>/lib/node_modules/`). `memarium upgrade` lands
in whichever `npm` runs first, but the shell resolves `memarium` by
PATH order — so users routinely upgrade one install while continuing
to run the other. Bit Yue twice on 2026-05-25 alone.

`doctor` now walks every `PATH` directory, lists every `memarium`
...
### `memarium sync` — orphan index entry prune

Pre-0.8.4, deleting a session from `~/.claude/projects/` (or its
Copilot equivalent) left its `.memarium/index.json` entry in place
...
had `.memarium/index.book.json`. The P7 raw_sessions aggregation
pass (0.8.0) sat below that gate, so on a setup where users have
synced raw_sessions but no one has run `/memarium` digest yet,
**`raw_sessions/` + `.memarium/index.aggregated.json` were never
written to main**.
...
The interactive `memarium init` wizard no longer asks "Encrypt raw
...
`.memarium/index.json` (full `displayName` per session) always leaked
...
- **At init**: `memarium init <url> --encrypt --passphrase <pp>`
  (non-interactive flag, unchanged from 0.5.x)
- **On an existing repo**: `memarium config --encrypt true` — flips
  config + re-wires the git crypt filter. Force-encrypt of already-
  plaintext blobs requires `rm -rf raw_sessions && memarium sync`.

### New: `memarium config --encrypt false`

Opt out of encryption on a previously-encrypted repo. Flips
`~/.memarium/config.json`'s `encrypt: false`, removes the per-clone
git crypt filter from `.git/config`, strips the
`raw_sessions/** filter=memarium` line from `.gitattributes` (leaving
...
everything now: `rm -rf raw_sessions && memarium sync`.
...
- **Device branches now inherit `.github/workflows/memarium-aggregate.yml`
  ...
  never triggers CI — and the user sees a fresh `memarium sync` push
  ...
  Root cause: `memarium workflow init` (0.5.3+) writes the yml to
  ...
  and `git show origin/main:.github/workflows/memarium-aggregate.yml`
...
raw_sessions aggregation**. Before this, `memarium resume <id>` could only
...
1. **`assets/scripts/merge-books.mjs` extension.** Besides aggregating
   ...
   `raw_sessions/` (via the device's `.memarium/index.json`), unions
   them into main (dedup by `tool:sessionId`, latest `sourceMtimeMs`
   wins, latest `originDevice` annotated), and writes
   `.memarium/index.aggregated.json` carrying the union with
...
2. **Sync overlay** (`src/aggregated-store.ts`, new). After a successful
   ...
   `~/.memarium/aggregated/` checked out on `main`. The worktree shares
   `.git` with `~/.memarium/session-repo/` so the smudge filter is
...
3. **`list-sessions` + `resume` updates.** Both now read BOTH the device's
   own `.memarium/index.json` AND the aggregated worktree's
...
- Next CI aggregate run (triggered by any device push) writes the new
  `.memarium/index.aggregated.json` + populates `raw_sessions/` on main.
- Next `memarium sync` on each device creates `~/.memarium/aggregated/`.
- `memarium list-sessions` then shows sibling-device sessions in the
  output; `memarium resume <id>` against them spawns Claude with the
  cross-device context.

No flags, no manual setup needed beyond `npm install -g memarium@0.8.0`.
...
`.memarium/index.json`. Default is dry-run; pass `--apply` to delete.
Empty parent dirs are removed after their last file goes away.

```sh
memarium prune           # list orphans (dry-run)
memarium prune --apply   # delete them
```
...
`memarium resume <id>` detects `manifest_version: 1` and switches to
...
### Companion: memarium-plugin SKILL.md P3 update

The `/memarium` write skill (separate `june9593/memarium-plugin` repo,
...
- **Claude extractor now filters `isMeta=true` entries.** These are
  ...
  started with `hi` + `/memarium` (both too short to survive the
  sanitizer's 10-char gate) would derive its displayName from the
  injected `/memarium` skill template, producing files like
  ...
  Yue's machine: ≥1 session per project that runs `/memarium` from a
  short opener.
...
- **`memarium workflow init` no longer fails when the user's primary
  ...
  fresh `memarium init` — local default branch is `main`). Failure was:
  `fatal: 'main' is already used by worktree at <repoPath>`. Fix:
  worktree now uses a unique temp branch name (`memarium-tmp-main-<ts>-<rand>`)
...
- **Q6 (Enable CI aggregation?) dropped** — now auto-true when
  ...
  the bug fix above). Escape hatch: edit `~/.memarium/config.json`
...
- **Closing message refreshed** to make cross-device flow obvious:
  "Try on this machine: memarium sync", then "On ANOTHER machine
  after memarium init + memarium sync: memarium list-sessions +
  memarium resume `<id>`".
...
### BREAKING — `memarium resume` mechanism changed
...
- `memarium resume <id-or-prefix> [--print] [--cwd <path>]` — shortId,
...
- `~/.memarium/resume-forks.json` registry (no fork tracking needed)
- `IndexEntry.originSessionId` field (no longer written; still read for
  back-compat)
- jsonl + raw.json in spool (no longer written)
- `memarium doctor` orphan-jsonl + oversized-jsonl checks (irrelevant
...
GitHub's 100 MB push limit. Override with `MEMARIUM_FULL_TOOL_RESULTS=1`.

### Migration

- First `memarium sync` after upgrade writes new-format `.md` *only* for
  sessions whose source jsonl mtime/sha changed. To force re-extract
  existing sessions in the new format:
  ```
  rm ~/.memarium/session-repo/.memarium/index.json
  memarium sync
  ```
- Old `.raw.json` / `.jsonl` files in the spool aren't auto-deleted;
  `memarium doctor` reports them with cleanup commands.
- `~/.memarium/resume-forks.json` from 0.5.1 is dead; `doctor` reports it.
- memarium-plugin needs no update — it already reads `.md` (just sees
  richer content now).
...
- **`memarium workflow init` now installs to `origin/main`** via a temp
  worktree (instead of the user's device branch). This means the very
  first push after `memarium init` triggers a fully-working CI run — no
  ...
  the new `memarium doctor` warning for the exact command.

- **`memarium init` wizard now actually installs the workflow** when Q6
  (enable CI aggregation) is answered yes. Previously it just set the
  config flag and asked you to run `memarium workflow init` later. Now
...
- **`merge-books.mjs` rendered strings are now localized.** The book
  ...
  `bookLocale: "en" | "zh"` in `~/.memarium/config.json` (default `"en"`),
  substituted into the workflow yml at install time as `MEMARIUM_LOCALE`.

- **`memarium doctor` flags workflow residue on the device branch.** Pre-
  0.5.3 users probably still have `.github/workflows/memarium-aggregate.yml`
...
1. `memarium upgrade` to 0.5.3 (or `npm i -g memarium@latest`).
2. `memarium workflow init` — this time it pushes to `origin/main`,
   fixing your CI run. (Idempotent: safe to run even if you already
   manually seeded `main`.)
3. (Optional, but recommended) `memarium doctor` will tell you if your
...
- **`memarium doctor` reports oversized jsonl.** Scans the spool for
...
- **`init` wizard adopts plugin-first directories.** If the user installed
  `memarium-plugin` first and the plugin wrote `book/` + `raw_sessions/`
...
- **Stable device branch name.** `os.hostname()` on macOS drifts across
  ...
  - **`memarium config --device <name>`**: existing users can fix their
    config after the fact. Output prints the `git branch -D / git push
    --delete` commands to clean up any drift-created branches.
  - **`memarium doctor`** flags drift-prone deviceBranch values.

### Migration note (0.4.x → 0.5.x)

If you're upgrading from a 0.4.x install, the first `memarium upgrade`
...
succeeded; **just run `memarium upgrade` a second time** and it works.
...
`memarium resume <sessionId>` now treats each resume as a **fork**:
...
- A new `~/.memarium/resume-forks.json` registry maps every freshly
  forked sessionId to its origin.
- The next `memarium sync` stamps the origin onto the spool's index
  entry as `originSessionId`, so plugin-side digest tooling can later
  group same-source threads.
- `memarium resume` output now shows the fork: `Session forked: abc123 → <new-uuid>`.

This closes the open design question from the v0.5.0 roadmap: "B 推回时怎么标记是 resumed-from-A".

## 0.5.0 — 2026-05-14

### BREAKING — Major slim, paired with the new memarium plugin

`memarium` 0.5.0 is **a deliberate amputation** of the npm package
to its sync transport responsibility. Digest + recall + the static
site renderer all moved to the Claude Code plugin at
[june9593/memarium-plugin](https://github.com/june9593/memarium-plugin).
The two products co-own the same `~/.memarium/session-repo/` spool
...
|---|---|
| `memarium prepare` | Plugin (`/memarium` skill drives this internally) |
| `memarium publish` | Plugin |
| `memarium recall` | Plugin (`/memarium-recall` skill) |
| `memarium serve` / `build-site` | Plugin (`/memarium` site rendering) |
| `memarium catalog-regen` | Plugin |
| `memarium list-projects` | Plugin |
| `memarium plugin-install` | Marketplace install: `/plugin install memarium` |
...
Your existing `~/.memarium/session-repo/` data continues to work
...
required by the new `memarium resume` command.
...
plugin's chronicle digest. Old ones trigger a `memarium doctor`
...
- `memarium list-sessions [--project --since --device]` — find resumable sessions across all devices in the spool.
- `memarium resume <sessionId>` — copy a spool session's jsonl into `~/.claude/projects/<encoded-cwd>/<id>.jsonl` and print the `cd <project> && claude --resume <id>` command to run.
- `memarium config --map-path FROM=TO` — register a cross-device path translation (e.g. `/Users/yueA=/Users/yueB`) used by `resume` to rewrite jsonl paths.
...
- `init-wizard` slimmed: 9 questions → 7. Closing message now suggests `/plugin install memarium` for digest + recall.
- `doctor` checks for the memarium plugin (`~/.claude/plugins/marketplaces/memarium-plugin/`) and prints a `warn` line if missing. Also warns about `.raw.json` files without sibling `.jsonl` (pre-0.5 sessions that can't be resumed).
- `upgrade` no longer runs `plugin-install` at the end — recommends `/plugin update memarium` instead.
